...
from utils.enums import TrainingAlgorithms, TaskType


class RandomForest:
    def __init__(self, nb_trees: int, max_depth: int, min_entropy: float, min_number_of_elem: int):
        ...
        self.subset_size = 1  # for bagging
        self.nb_classes = 0
        self.trees = []
    ...
    def train(self, training_algorithm, inputs: np.ndarray, targets: np.ndarray, nb_classes: int,
        ...
        for i in range(self.nb_trees):
            decision_tree = DT(task_type=TaskType.classification,
                               ...
                               min_number_of_elem=self.min_number_of_elem)
